...
import depol from '../../server/data/depol.json';
import pm10 from '../../server/data/pm10.json';
import pm25 from '../../server/data/pm25.json';
...
export default () => {
  ...
  const fetchData = useCallback((index: number) => {
    setLoading(true);
    const key = dataTypeSet[index].key;
    setTimeout(() => {
      let source = null;
      if (key === 'depol') {
        source = parse(depol.data.list, key);
      } else if (key === 'pm10') {
        source = parse(pm10.data.list, key);
      } else if (key === 'pm25') {
        source = parse(pm25.data.list, key);
      }
      if (source) {
